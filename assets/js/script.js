...
document.addEventListener('visibilitychange',
    function () {
        if (document.visibilityState === "visible") {
            document.title = "Portfolio | Rohit Jalakam";
...
function showProjects(projects) {
    ...
    projects.slice(0, 10).filter(project => project.category != "android").forEach(project => {
                // support image names with or without extension
                const imgSrc = (project.image && project.image.includes('.')) ?
                    `/assets/images/projects/${project.image}` :
                    `/assets/images/projects/${project.image}.png`;

                // safe link fallbacks in case JSON entry misses links
                const viewLink = project.links && project.links.view ? project.links.view : '#';
                const codeLink = project.links && project.links.code ? project.links.code : '#';
